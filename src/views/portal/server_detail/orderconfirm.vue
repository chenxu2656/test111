...
const createAddress = async () => {
  const addressvalue = getAddressByCode(
    address.value.address[address.value.address.length - 1],
  );
  const postObj = {
    user_id:
  ...
  await http
    .request<any>("post", "/api/v1/address", {
      data: postObj,
    })
    .then((res) => {
...
const getaddressList = async () => {
  await http
    .request<any>("get", "/api/v1/address", {
      params: {
        user_id:
          JSON.parse(localStorage.getItem("userInfo"))?.id ||
          "ddc42355-d9ca-4a75-ae2b-5ea4ba344b73",
...
const createOrder = async () => {
  ...
  await http
    .request<any>("post", `/api/v1/orders?backurl=${backUrl}`, {
...
<template>
  ...
  <div class="container">
    <div class="titl" style="font-size: 18px; font-weight: 500">
      确认订单信息
    </div>
    ...
    <el-card style="margin-top: 20px">
      <div class="headercon flex justify-between">
        <div class="header flex" style="align-items: center">
          <div
            style="width: 4px; height: 20px; background-color: #13ae68"
          ></div>
          <div class="tit ml-2" style="font-size: 15px; font-weight: 500">
            收货地址
          </div>
    ...
    <div class="action my-10 flex justify-end">
      <div class="price">
        <span>应付价格:</span>
        <span style="font-size: 19px; color: red; font-weight: 500"
          >¥{{ orderInfo.total_amount }}</span
        >
      </div>
      <el-button
        style="width: 150px"
