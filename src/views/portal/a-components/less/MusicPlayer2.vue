...
export default {
  ...
  props: {
    ...
    backSecond: { type: Number, default: 3 },
    forwardSecond: { type: Number, default: 10 }
  },
  data() {
    return {
      duration: '00:00',
  ...
  methods: {
    ...
    updateTime() {
      if (!this.$refs.progress)
        returnthis.currentDuration = this.timeFormat(this.audio.currentTime)
    ...
    handleMouseup() {
      setTimeout(() => {
        this.audio.play()
        this.paused = falsethis.isMoveIn = false
      }, 200)
    },
    //小圆圈按下
    handleMousedown() {
      this.audio.pause()
      this.paused = truethis.isMoveIn = true
    ...
    <div class="card">
      ...
      <div class="option">
        <span class="pre" @click="handleBack"
          ><img src="../../assets/svg/logoV.svg" alt="" />
          <span style="color: #c7c7c9; font-size: 13px; margin-left: 10px"
            >-{{ backSecond }}s</span
          ></span
        >
        <span class="play" @click="handlePauseOrPlay"
          ><el-icon size="35px" color="#3853FE"
            ><video-pause v-show="!paused" /><video-play
              v-show="paused" /></el-icon></span
        ><span class="next" @click="handleForward"
          ><span style="color: #c7c7c9; font-size: 13px; margin-right: 10px"
            >+{{ forwardSecond }}s </span
          ><img src="../../assets/svg/logoV.svg" alt="dddd"
        /></span>
      </div>
    </div>
  </div>
